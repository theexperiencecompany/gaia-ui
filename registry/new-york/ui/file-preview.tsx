...
const getFileIcon = (fileType: string, fileName: string) => {
  ...
  if (fileType.startsWith("image/"))
    return <FileImage className={cn(iconClass, "text-emerald-500 dark:text-emerald-400")} />;
  ...
  if (
    ...
    return <FileText className={cn(iconClass, "text-blue-500 dark:text-blue-400")} />;
  ...
  if (
    ...
    return <FileText className={cn(iconClass, "text-green-500 dark:text-green-400")} />;

  // Code/text files
  if (["txt", "md"].includes(extension) || fileType === "text/plain")
    return <FileText className={cn(iconClass, "text-zinc-500 dark:text-zinc-400")} />;

  if (
    [
      "js", "ts", "jsx", "tsx", "py", "java", "c", "cpp", "html", "css"
    ].includes(extension) ||
    fileType.includes("javascript") ||
    fileType.includes("typescript")
  )
    return <FileCode className={cn(iconClass, "text-yellow-500 dark:text-yellow-400")} />;

  if (["json", "xml", "yaml", "yml"].includes(extension))
    return <FileCode className={cn(iconClass, "text-zinc-500 dark:text-zinc-400")} />;
  ...
  if (
    ...
    return <FileVideo className={cn(iconClass, "text-purple-500 dark:text-purple-400")} />;

  if (
    fileType.startsWith("audio/") ||
    ["mp3", "wav", "ogg"].includes(extension)
  )
    return <FileAudio className={cn(iconClass, "text-pink-500 dark:text-pink-400")} />;
  ...
  if (
    ...
    return <FileArchive className={cn(iconClass, "text-amber-500 dark:text-amber-400")} />;

  // Default fallback
  return <File className={cn(iconClass, "text-zinc-500 dark:text-zinc-400")} />;
};
        ...
        {files.map((file) => (
          ...
          >
            ...
            {onRemove && (
              ...
              >
                <X className="h-3 w-3 text-zinc-700 dark:text-white" />
              </button>
            )}

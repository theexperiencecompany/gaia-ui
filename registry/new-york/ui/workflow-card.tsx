...
const toolCategoryConfig: Record<string, { bgColor: string; iconColor: string }> = {
  productivity: { bgColor: "bg-emerald-500/20", iconColor: "text-emerald-600 dark:text-emerald-400" },
  documents: { bgColor: "bg-orange-500/20", iconColor: "text-orange-600 dark:text-orange-400" },
  development: { bgColor: "bg-cyan-500/20", iconColor: "text-cyan-600 dark:text-cyan-400" },
  memory: { bgColor: "bg-indigo-500/20", iconColor: "text-indigo-600 dark:text-indigo-400" },
  creative: { bgColor: "bg-pink-500/20", iconColor: "text-pink-600 dark:text-pink-400" },
  goal_tracking: { bgColor: "bg-emerald-500/20", iconColor: "text-emerald-600 dark:text-emerald-400" },
  notifications: { bgColor: "bg-yellow-500/20", iconColor: "text-yellow-600 dark:text-yellow-400" },
  email: { bgColor: "bg-blue-500/20", iconColor: "text-blue-600 dark:text-blue-400" },
  calendar: { bgColor: "bg-purple-500/20", iconColor: "text-purple-600 dark:text-purple-400" },
  search: { bgColor: "bg-teal-500/20", iconColor: "text-teal-600 dark:text-teal-400" },
  general: { bgColor: "bg-gray-500/20", iconColor: "text-gray-600 dark:text-gray-400" },
};
...
const CategoryIcon: FC<{ category: string; className?: string }> = ({ category, className }) => {
  const config = toolCategoryConfig[category] || toolCategoryConfig.general;
  
  return (
    <div className={cn("rounded-lg p-1.5", config.bgColor, className)}>
      <div className={cn("h-5 w-5 rounded", config.iconColor)}>
  ...
  return (
    <div
      className={cn(
        ...
        onClick && "cursor-pointer hover:bg-zinc-200/50 dark:hover:bg-zinc-700/50",
        className
      )}
      onClick={onClick}
      onKeyDown={(e) => {
    ...
    >
      ...
      <div>
        <h3 className="line-clamp-2 text-lg font-medium text-zinc-900 dark:text-white">{title}</h3>
